      ...
      if (s != "")
      {
        ...
        if (event == "telemetry")
        {
          ...
          */
          ...
          Eigen::VectorXd ptscx(ptsx.size());
          Eigen::VectorXd ptscy(ptsy.size());

          // from word coordinates to car coordinates
          for (int i = 0; i < ptsx.size(); i++)
          {
            double x = ptsx[i] - px;
            double y = ptsy[i] - py;
            ptscx[i] = x * cos(-psi) - y * sin(-psi);
            ptscy[i] = x * sin(-psi) + y * cos(-psi);
          }
          ...
          */
          ...
          auto coeffs = polyfit(ptscx, ptscy, 3);
          ...
          double pred_v = v + acc * latancy;
          double pred_cte = cte + v * sin(epsi) * latancy;
          double pred_epsi = epsi + v * -delta / Lf * latancy;

          // Feed in the predicted state values
          Eigen::VectorXd state(NState);
          ...
          vector<double> mpc_x_vals = {pred_px};
          vector<double> mpc_y_vals = {pred_py};
